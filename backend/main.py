...
EMAIL_FROM       = os.getenv("EMAIL_FROM", "Tone <onboarding@resend.dev>")
...
def _html_to_pdf(html: str) -> bytes:
    chrome = next((p for p in _CHROME_CANDIDATES if p and Path(p).exists()), None)
    if not chrome:
        raise RuntimeError("Headless browser (Chrome/Edge) not found for PDF export.")
...
app = FastAPI(title="Tone — Water Intelligence API", description="Precision Droplet & Commercial Water Reuse Intelligence Engine")
...
@app.get("/api/buildings/{building_id}", response_model=BuildingRecord)
...
@app.post("/api/email-brief", status_code=200)
def email_investment_brief(req: EmailBriefRequest):
    ...
    try:
        resend.Emails.send({
            "from": EMAIL_FROM,
            "to": [req.email],
            "subject": f"Tone Investment Brief — {building_ref} · {building.address.split(',')[0]}",
            "html": (
                f"<p>Hi,</p>"
                f"<p>Please find attached the Tone AI Investment Brief for "
            ...
            "attachments": [{
                "filename": f"Tone_Brief_{building_ref}.pdf",
                "content": pdf_b64,
            }],
        })
    except Exception as e:
